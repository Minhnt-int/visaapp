...
import { VisaPageVisaType, PricingTypeItem, PricingDetails } from '@/types';
...
export function VisaTabs({ visaTypes, pricing, countryName }: VisaTabsProps) {
  ...
  const [activeVisaType, setActiveVisaType] = useState(visaTypes[0]?.id || '');
  const [activePriceType, setActivePriceType] = useState(pricing[0]?.type || '');

  // Update active type when visaTypes or pricing props change
  // This handles cases where initial data might not have the default 'du-lich' type
  useState(() => {
    if (visaTypes.length > 0 && !visaTypes.some(type => type.id === activeVisaType)) {
      setActiveVisaType(visaTypes[0].id);
    }
    if (pricing.length > 0 && !pricing.some(type => type.type === activePriceType)) {
      setActivePriceType(pricing[0].type);
    }
  });
...
function PricingTab({
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-8">
      ...
      {/* Fee Breakdown */}
      {currentPricing.prices && currentPricing.prices.length > 0 && currentPricing.prices[0].consularFee && currentPricing.prices[0].serviceFee && (
