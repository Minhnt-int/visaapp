...
export default function Header({navigationLinks} : {navigationLinks : NavItem[]}) {
  ...
  return (
    <>
      ...
      {/* Main Header */}
      <header className="sticky top-0 z-30 w-full border-b border-neutral-300 bg-background/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center flex-1 justify-center">
            {navigationLinks.map((link) => (
              <div key={link.href} className="relative group" onMouseLeave={() => setOpenDropdown(null)}>
                <Link href={link.href} className="flex items-center gap-1 px-4 py-2 text-neutral-700 hover:text-primary font-medium transition-colors" onMouseEnter={() => link.children && setOpenDropdown(link.href)}>
                  {link.label}
                  {link.children && <ChevronDown size={16} />}
                </Link>
                {link.children && (
                  <div className={`absolute top-full left-0 w-64 bg-background shadow-lg border rounded-lg py-2 z-40 transition-all duration-200 ${openDropdown === link.href ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                    {link.children.map((child) => (
                      <div key={child.href} className="relative group/sub" onMouseLeave={() => setOpenSubDropdown(null)}>
                        <Link href={child.href} className="flex items-center justify-between px-4 py-2 text-neutral-700 hover:bg-neutral-100" onMouseEnter={() => child.children && setOpenSubDropdown(child.href)}>
                          { visaContinentsSlugtoName(child.label)}
                          {child.children && child.children.length > 0 && <ChevronRight size={16} />}
                        </Link>
                        {child.children && child.children.length > 0 && (
                          <div className={`absolute top-0 left-full w-64 bg-background shadow-lg border rounded-lg py-2 z-50 transition-all duration-200 ${openSubDropdown === child.href ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                            {child.children.map((subChild) => (
                              <Link key={subChild.href} href={subChild.href} className="block px-4 py-2 text-neutral-700 hover:bg-neutral-100">{subChild.label}</Link>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          <div className="flex items-center gap-2">
            <SearchContainer />
            <div className="lg:hidden"><button onClick={toggleMenu} className="p-2 text-neutral-700 hover:text-primary" aria-label="Toggle menu">{isMenuOpen ? <X size={24} /> : <Menu size={24} />}</button></div>
          </div>
        </div>

        {/* Mobile Menu - Also simplified */}
        <div className={`lg:hidden ${isMenuOpen ? 'block' : 'hidden'} bg-background border-t border-neutral-200`}>
          <nav className="container mx-auto px-4 pb-4 mt-4 space-y-2">
            {navigationLinks.map((link) => (
              <div key={link.href}>
                <div className="flex justify-between items-center">
                  <Link href={link.href} className="flex-1 py-2 font-medium text-neutral-800">{link.label}</Link>
                ...
                {link.children && openDropdown === link.href && (
                  <div className="pl-4 border-l-2 border-neutral-200 ml-2">
                    {link.children.map((child) => (
                      <div key={child.href}>
                        <div className="flex justify-between items-center">
                          <Link href={child.href} className="flex-1 py-2 text-neutral-700">{child.label}</Link>
                        ...
                        {child.children && child.children.length > 0 && openSubDropdown === child.href && (
                          <div className="pl-4 border-l-2 border-neutral-300 ml-2">
                            {child.children.map((subChild) => (
                              <Link key={subChild.href} href={subChild.href} className="block py-2 text-neutral-600">{subChild.label}</Link>
                            ))}
                          </div>
                        )}
                      </div>
